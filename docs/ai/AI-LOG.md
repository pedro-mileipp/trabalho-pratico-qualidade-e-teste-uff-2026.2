...
| Alexandre Colmenero | Caso de teste manual — PIX (issue #8) | opencode (modelo `opencode-go/deepseek-v4-flash`) | [Sessão 6 — prompts](prompts/sessao-6-caso-manual-pix.md) | Caso manual do PIX projetado e **executado** via Selenium/Chrome headless: 12 casos (TC-01 a TC-12) com 14 screenshots em `tests/testes_manuais/pix/evidencias/`; resultado **11 PASS e 1 FAIL** — o FAIL confirma o defeito **#16** (e-mail com dois `@` aceito); documentado em `tests/testes_manuais/pix/caso_pix.md`, linkado no README e entregue via **PR #18** (fecha #8) | Aceito; limitações registradas (valor ≤ 0 barrado por HTML5; limite noturno às 22h; chave bloqueada não alcançável via UI) | Resultados confirmados pelo corpo HTML das respostas; `gh issue view 8` conferindo os critérios de aceite; arquivos de evidência conferidos; PR #18 aberto |
