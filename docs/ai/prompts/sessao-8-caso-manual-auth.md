...
> "Pensei nos seguintes casos de teste manuais. 1) tentar logar sem ter uma conta; 2) cadastrar uma conta e logar; 3) clicar em sair para realizar o logout; 4) logar tendo conta já cadastrada; 5) tentar logar com e-mail e senha inválidos; 6) tentar cadastrar conta com e-mail e senha inválidos; 7) tentar criar outra conta com os mesmos dados já utilizados; 8) tentar não utilizar @; 9) tentar não passar nada depois do @."
>
...
2. **Revisão da cobertura de testes**
   - Confirmou que os 9 cenários propostos pela usuária cobriam o fluxo principal, mas havia lacunas de borda.
...
|---|---|---|
| Mapeamento inicial dos cenários de autenticação e registro, focados em login, cadastro e logout | Suíte manual ampliada com 11 cenários executados, incluindo validações de borda e recuperação de fluxo após logout | A IA revisou os casos propostos, identificou lacunas (campos vazios, e-mail incompleto, senha curta) e complementou a suíte com 2 cenários adicionais |
...
- A suíte executada cobriu 11 cenários, incluindo os 9 propostos pela usuária e 2 adicionais identificados pela IA.
